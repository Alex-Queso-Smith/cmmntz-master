...
export const FilterFromButton = props => {
  return(
    <span className="cf-filter-btn-container">
      <button name={props.name} value={props.value} className={`btn btn-sm cf-dark-button cf-filter-btn ${props.extraClass} ${props.opacityClass}`} onClick={props.onClick}>{props.title}</button>
    </span>
  )
}
